...
import { App, Avatar, Button, Drawer, Input, Modal, Select, Space, Spin, Table, Tabs, Tag, Typography } from "antd"
...
const PostsPage: React.FC = () => {
  ...
  const renderStatusTags = (post: Post) => {
    ...
    if (post.isPinned)
      tags.push(
        <Tag key="pin" color="gold">
          置顶
        </Tag>,
      )
    if (post.isHighlighted)
      tags.push(
        <Tag key="highlight" color="orange">
          精华
        </Tag>,
      )
    if (post.isLocked)
      tags.push(
        <Tag key="lock" color="volcano">
          锁定
        </Tag>,
      )
    if (post.isHidden)
      tags.push(
        <Tag key="hidden" color="red">
          隐藏
        </Tag>,
      )
    if (tags.length === 0)
      tags.push(
        <Tag key="normal" color="blue">
          正常
        </Tag>,
      )
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space wrap>
          ...
          <Button size="small" icon={<PushpinOutlined />} type="link" onClick={() => handleTogglePin(record)}>
            {record.isPinned ? "取消置顶" : "置顶"}
          </Button>
          <Button size="small" icon={<HighlightOutlined />} type="link" onClick={() => handleToggleHighlight(record)}>
            {record.isHighlighted ? "取消加精" : "加精"}
          </Button>
          <Button
            size="small"
          ...
          <Button size="small" icon={<StopOutlined />} type="link" onClick={() => handleToggleHidden(record)}>
            {record.isHidden ? "取消隐藏" : "隐藏"}
          </Button>
          <Button size="small" icon={<DeleteOutlined />} danger type="link" onClick={() => handleDelete(record.id)}>
            删除
          </Button>
        </Space>
      ),
  ...
  return (
    <div className="space-y-4">
      ...
      <Drawer open={!!detailPost} width={600} title="帖子详情" onClose={() => setDetailPost(null)}>
        {detailPost && (
          <Tabs
            items={[
              {
                    ...
                    <Space direction="vertical" size={16} className="w-full">
                      ...
                      <div>
                        ...
                        <Space align="center">
                          <Avatar
                            src={getAvatarSrc(
                              detailPost.author.nickname || detailPost.author.username,
                      ...
                      <div className="flex gap-2 flex-wrap">
                        <Button size="small" onClick={() => handleTogglePin(detailPost)} icon={<PushpinOutlined />}>
                        ...
                        >
                          {detailPost.isHighlighted ? "取消加精" : "加精"}
                        </Button>
                        <Button
                          size="small"
                      ...
                      <div>
                        ...
                        <Space>
                          <Tag color="blue" bordered={false}>
                            浏览 {detailPost.viewCount}
                          </Tag>
                          <Tag color="geekblue" bordered={false}>
                            评论 {detailPost.commentCount}
                          </Tag>
                          <Tag color="cyan" bordered={false}>
                            点赞 {detailPost.likeCount}
                          </Tag>
                        </Space>
                      </div>
                      <div>
                      ...
                      <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                          ...
                          <div>
                            {detailPost.updatedAt ? new Date(detailPost.updatedAt).toLocaleString("zh-CN") : "暂无"}
                          </div>
                        </div>
                      </div>
                      <div>
                      ...
                      columns={[
                        ...
                        },
                        {
                          ...
                          render: (author: Comment["author"]) => (
                            <Space align="center">
                              <Avatar size={28} src={getAvatarSrc(author.nickname || author.username, author.avatar)} />
                              <div>
